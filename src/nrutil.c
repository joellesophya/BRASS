...
//long nch,ncl,ndh,ndl,nrh,nrl,nil,nih;
...
double ****d4tensor(long nrl, long nrh, long ncl, long nch, long ndl, long ndh, long nil, long nih)
/* allocate a double 4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nil..nih] */
{
	long i,j,k,nrow=nrh-nrl+1,ncol=nch-ncl+1,ndep=ndh-ndl+1, nidim=nih-nil+1;
	double ****t;
	...
	/* allocate the 4th dimentions, i, and set pointers to them */
	t[nrl][ncl][ndl]=(double *) malloc((unsigned int)((nrow*ncol*ndep*nidim+NR_END)*sizeof(double)));
	if (!t[nrl][ncl][ndl]) nrerror("allocation failure 4 in f4tensor()");
	t[nrl][ncl][ndl] += NR_END;
	t[nrl][ncl][ndl] -= nil;
	
	/* initially only t[nrl][ncl][ndl], t[nrl][ncl] and t[nrl] are valid */
	
	
	/* now set t[nrl][ncl][*] */
	for (k = ndl+1; k <= ndh; k++) {
		t[nrl][ncl][k] = t[nrl][ncl][k-1] + nidim;
	}
	/* t[nrl][ncl][*] is valid */
	
	/* now set t[nrl][*][*] and t[nrl][*] */
	for (j = ncl+1; j <= nch; j++) {
		t[nrl][j] = t[nrl][j-1]+ndep;
		/* now t[nrl][*] is valid */
		
		t[nrl][j][ndl] = t[nrl][j-1][ndl]+ndep*nidim;
		/* now t[nrl][*][ndl] is valid */
		for (k = ndl+1; k <= ndh; k++) {
			t[nrl][j][k] = t[nrl][j][k-1]+nidim;
		}
		/* now t[nrl][*][*] is valid */
	}
	...
	for (i = nrl+1; i <= nrh; i++) {
		...
		t[i][ncl][ndl] = t[i-1][ncl][ndl] + nidim*ncol*ndep;
		/* now t[*][ncd][ndl] is valid */
		
		for (k = ndl+1; k <= ndh; k++) {
			t[i][ncl][k] = t[i][ncl][k-1] + nidim;
			/* now t[*][ncl][*] is valid */
		}
		...
		for (j = ncl+1; j <= nch; j++) {
			...
			t[i][j][ndl] = t[i][j-1][ndl] + ndep*nidim;
			/* now t[*][*][ndl] is valid */
			for (k = ndl+1; k <= ndh; k++) {
				t[i][j][k] = t[i][j][k-1] + nidim;
			}
			/* now t[*][*][*] is valid */
		}
	}
...
/* allocate an unsigned long vector with subscript range v[nl..nh] */
{
	...
	v=(unsigned long *)malloc((unsigned int) 
((nh-nl+1+NR_END)*sizeof(long)));
	if (!v) nrerror("allocation failure in lvector()");
	return v-nl+NR_END;
}
